...
// 获取说说列表
export const getRecordListAPI = (data?: QueryData) =>
  Request<Record[]>('POST', `/record/list`, {
    data: { ...data?.query },
  });

// 分页获取说说列表
export const getRecordPagingAPI = (data?: QueryData) =>
  Request<Paginate<Record[]>>('POST', `/record/paging`, {
    data: { ...data?.query },
    params: {
      ...data?.pagination,
    },
