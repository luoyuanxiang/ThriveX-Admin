...
// 获取留言列表
export const getWallListAPI = (data?: QueryData) =>
  Request<Wall[]>('POST', `/wall/list`, {
    data: { ...data?.query },
  });

// 分页获取留言列表
export const getWallPagingAPI = (data?: QueryData) =>
  Request<Paginate<Wall[]>>('POST', `/wall/paging`, {
    data: { ...data?.query },
    params: {
      ...data?.pagination,
    },
