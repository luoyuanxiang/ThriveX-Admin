...
// 获取相册列表
export const getAlbumCateListAPI = (data?: QueryData) =>
  Request<AlbumCate[]>('POST', '/album/cate/list', {
    data: { ...data?.query },
  });

// 分页获取相册列表
export const getAlbumCatePagingAPI = (data?: QueryData) =>
  Request<Paginate<AlbumCate[]>>('POST', `/album/cate/paging`, {
    data: { ...data?.query },
    params: {
      ...data?.pagination,
    },
