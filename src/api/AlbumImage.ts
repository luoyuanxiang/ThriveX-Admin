...
// 获取照片列表
export const getAlbumImageListAPI = (data?: QueryData) =>
  Request<AlbumImage[]>('POST', '/album/image/list', {
    data: { ...data?.query },
  });

// 分页获取照片列表
export const getAlbumImagePagingAPI = (data?: QueryData) =>
  Request<Paginate<AlbumImage[]>>('POST', `/album/image/paging`, {
    data: { ...data?.query },
    params: {
      ...data?.pagination,
    },
