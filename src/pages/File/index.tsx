...
export default () => {
  ...
  const getFileList = async (dir: string, isLoadMore = false) => {
    ...
    try {
      ...
      if (!isLoadMore) {
        ...
        setFileList((prev) => [...prev, ...data.records]);
        setPage((prev) => prev + 1);
      }

      // 判断是否还有更多数据
      setHasMore(data.records.length === 15);
   ...
   * @param data 要下载的文件数据
   */
  const onDownloadImage = async (data: File) => {
    try {
        ...
        <Spin spinning={loading}>
          <div
            className={`flex flex-wrap ${
              dirName ? '!justify-center' : 'justify-start!'
